...
for CH in 256 512 1024 2048 4096; do
  clang -O3 -mcpu=native -Xclang -fopenmp -I"$OMP/include" -w \
        -DLAGUNA_METAL -DLG_CHUNK=$CH \
        c/laguna_xs.c c/laguna_metal.o -o /tmp/lxs_ch \
        -lm -L"$OMP/lib" -lomp \
        -framework Metal -framework MetalPerformanceShaders -framework Foundation \
        -lc++ -lobjc 2>/dev/null
  OUT=$(SNAP="$MODEL" /tmp/lxs_ch 0 0 --chat -n 4 -f "$PROMPT" 2>&1 | tr '\r' '\n')
